...
export default function AdSlot({ slot, format = 'horizontal', className = '' }: Props) {
  ...
  if (!client) {
    return (
      <div
        className={`flex items-center justify-center border border-dashed border-zinc-700 bg-zinc-900 rounded-lg text-zinc-500 text-xs ${FORMAT_CLASSES[format]} ${className}`}
        aria-hidden="true"
      >
        Ad · {format} · slot {slot}
      </div>
